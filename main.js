...
function addProgress(amount = 1) {
  ...
  uiContainer.innerHTML += `<span id="point-${pointUpId}" style="left: ${mousePointer.clientX - 3}px;top: ${
    mousePointer.clientY - 5
  }px;" class="plus-point-nudge">+ 1</span>`;
  gsap.fromTo(
...
function RaycasterRender() {
  ...
  if (trashesHovered.length > 0 && (KeyActive.leftClick || KeyActive.touch)) {
    const object = trashes.filter(
      (x) => x.trash.children[0].children[0].children[0].children[0].children[0].uuid == trashesHovered[0].object.uuid
    )[0];
    object.removed = true;
